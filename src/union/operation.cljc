(ns union.operation
  "OperationActor -- one trade-union operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (UnionOps-LLM) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the Union Governance Governor (:govern) and
  ...
  One graph run = one trade-union operation (intake -> advise -> govern
  ...
  decision to a human operator (elected union leadership). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/authorize-strike`/`:actuation/finalize-
  bargaining-position` ALWAYS reach this node when the governor is
  clean -- see `union.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [union.unionadvisor :as unionadvisor]
            [union.governor :as governor]
...
(defn build
  ...
  `union.store/Store`).
  opts:
    :advisor      -- a `union.unionadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (unionadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; UnionOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (unionadvisor/-advise advisor store request)]
            {:proposal p :audit [(unionadvisor/trace request p)]})))

      ;; Union Governance Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
